...
class Purchases_Validator_IM_HC_MVC extends _HC_Validator
{
	public function prepare( $values )
	{
		...
		if( ! $app_settings->get('purchases:ref_auto_gen') ){
			$return['ref']['required'] = array( $this->make('/validate/required') );
		}
		$return['ref']['maxlen'] = array( $this->make('/validate/maxlen'), 100 );
		$return['ref']['unique'] = array( $this->make('/validate/unique'), 'purchases', 'ref', $id );

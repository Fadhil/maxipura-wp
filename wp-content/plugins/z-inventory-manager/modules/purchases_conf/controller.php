...
class Purchases_Conf_Controller_IM_HC_MVC extends _HC_Form
{
	...
	public function settings_before_get( $args, $src )
	{
		...
		switch( $pname ){
			case 'purchases:next_ref':
				$auto_gen = $src->run('get', 'purchases:ref_auto_gen');

				if( $auto_gen ){
					$prefix = $src->run('get', 'purchases:ref_prefix');
					$random = $src->run('get', 'purchases:ref_number_random');

...
class Receives_Purchases_Input_Items_IM_HC_MVC extends HC_Form_Input2
{
		...
		if( $value ){
			foreach( $value as $item_id => $item_details ){
				...
				$item_view = $this->make('/html/view/grid')
					...
					->add( $item_conf['need'] + $item_conf['received'],		2 )
					->add( $item_conf['received'],	2 )
					->add( $input_qty,				2 )
					;
